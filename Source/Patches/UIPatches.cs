...
namespace ConfigurableWarning.Patches {
    ...
    internal class UIPatches {
        ...
        internal static void UpdateHealth(UI_Health __instance) {
            __instance.fill.fillAmount = Player.localPlayer.data.health / Plugin.Instance.PluginSettings.maxHealth.Value;
        }

        [HarmonyPostfix]
        [HarmonyPatch(typeof(UI_DaysLeft), nameof(UI_DaysLeft.Update))]
        internal static void UpdateDaysLeft(UI_DaysLeft __instance) {
            int num = Plugin.Instance.PluginSettings.daysPerQuota.Value - SurfaceNetworkHandler.RoomStats.CurrentQuotaDay + 1;
